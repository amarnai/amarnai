...
import {
  ...
  tokenize,
  MAX_CANDIDATE_PATHS,
} from "../selection/candidate-selector.js";
...
describe("selectCandidateNodes — valid taxonomy", () => {
  ...
  it("scores higher when email matches node name (weight 3)", () => {
    ...
    const result = selectCandidateNodes(BASE_NODES, BASE_EDGES, emails);
    const clientsCandidate = result.candidates.find((c) => c.nodeId === "clients");
    const financeCandidate = result.candidates.find((c) => c.nodeId === "finance");
    ...
    expect(result.candidates[0]!.nodeId).toBe("clients");
  });
  ...
  it("description match contributes to score (weight 2)", () => {
    ...
    const result = selectCandidateNodes(nodes, edges, emails);
    const aCandidate = result.candidates.find((c) => c.nodeId === "a");
    const bCandidate = result.candidates.find((c) => c.nodeId === "b");
  ...
  it("ancestor name match contributes to score (weight 1)", () => {
    ...
    const result = selectCandidateNodes(nodes, edges, emails);
    const candidate = result.candidates.find((c) => c.nodeId === "clientleaf");
  ...
  it("handles cycles in edges without hanging", () => {
    ...
    const result = selectCandidateNodes(nodes, edges, []);
  ...
  it("caps results at MAX_CANDIDATE_PATHS", () => {
    ...
    const result = selectCandidateNodes([ROOT, ...leafNodes], leafEdges, []);
  ...
  it("promotes a fallback-named node to the last slot when it would be cut off", () => {
    ...
    expect(lastCandidate.nodeId).toBe("fallback");
