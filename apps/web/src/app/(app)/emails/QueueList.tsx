...
import { QUEUES, countForActive } from "./selection";
...
type Props = {
  ...
  railQuery: string;
  now: Date;
  onSelect: (id: ActiveSelection) => void;
};
...
export function QueueList({
  ...
  const visible = q
    ? QUEUES.filter((x) => x.name.toLowerCase().includes(q))
    : QUEUES;
  ...
  return (
    <>
      {visible.map((queue) => {
        const isActive =
          active.kind === "queue" && active.id === queue.id;
        const count = countForActive(
          threads,
          folders,
          { kind: "queue", id: queue.id },
          now
        );
        return (
          <button
