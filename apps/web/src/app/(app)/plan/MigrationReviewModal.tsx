 ...
 */
export function MigrationReviewModal({ file, preview, submitting, onCancel, onConfirm }: Props) {
  const { _ } = useLingui();

  // New (incoming) non-root folders, minus the catch-all (never a manual target).
  const newFolders = useMemo(
    () => file.nodes.filter((n) => !n.isRoot && !(n.isCatchAll ?? false)).map((n) => ({ ref: n.ref, name: n.name })),
    [file],
  );
  ...
  const resortLive =
    fixedReviewResort +
    editableRows.filter((s) => choices[s.oldNodeId] === RESORT).reduce((sum, s) => sum + s.threadCount, 0);
    ...
    >
      <div className="modal" style={{ maxWidth: 560 }}>
        <div className="modal-header">
          ...
          <button className="modal-close" aria-label={_(msg`Cancel`)} onClick={onCancel} disabled={submitting}>
            ✕
          </button>
        </div>
        ...
        <div className="modal-body">
          <p style={{ marginBottom: 12 }}>
            <Trans>
              Choose where each folder&rsquo;s threads should go under your new plan.
              Matched folders keep their threads instantly; anything set to re-sort
              is re-classified by AI.
            </Trans>
          </p>

          {editableRows.length === 0 && !catchAllRow && (
            <p style={{ color: "var(--color-muted)", fontSize: 13 }}>
              <Trans>No sorted threads to migrate. Your new plan will apply directly.</Trans>
            </p>
          )}
          ...
          {catchAllRow && (
            <p style={{ fontSize: 11, color: "var(--color-muted)", marginTop: 10 }}>
              <Trans>
                Automated and bulk mail ({catchAllRow.threadCount}) always moves to the new catch-all folder.
              </Trans>
            </p>
          )}
          ...
          {hiddenCount > 0 && (
            <p style={{ fontSize: 11, color: "var(--color-muted)", marginTop: 6 }}>
              <Plural value={hiddenCount} one="# folder with no threads is not shown." other="# folders with no threads are not shown." />
            </p>
          )}
          ...
          <div
            style={{
              ...
              padding: "10px 12px",
              borderRadius: 8,
              background: "var(--color-surface-2, var(--color-muted-bg, rgba(0,0,0,0.04)))",
            }}
          >
            <p style={{ margin: 0 }}>
              <Trans>
            ...
            <p style={{ margin: "6px 0 0", color: "var(--color-muted)", fontSize: 12 }}>
              <Trans>
                Re-sorting threads already sorted this month is free. If you reach your
                monthly limit, the rest resume next month.
              </Trans>
            </p>
          </div>
        </div>
