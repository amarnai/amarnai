...
| `EMBEDDING_PROVIDER`          | `mock`       | `frontier` for production, `ollama` for local dev, `mock` for tests |
...
| `FRONTIER_EMBEDDING_MODEL`    | —            | Embedding model name (e.g. `text-embedding-004`, `text-embedding-3-small`) |
