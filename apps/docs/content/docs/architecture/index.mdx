---
title: Architecture
description: High-level overview of how Aziru is built.
---

Aziru is a TypeScript monorepo managed with [pnpm workspaces](https://pnpm.io/workspaces) and [Turborepo](https://turbo.build/). It is composed of several applications and a set of shared packages. The runtime services are `web`, `api`, and `worker`; the remaining apps are supporting surfaces (marketing site, browser extension, docs) and a shelved mobile app.
...
**Threads, not messages.** Aziru triages email threads as a unit. A new message in an existing thread triggers re-triage of the whole thread, so the sort order always reflects the full conversation.
