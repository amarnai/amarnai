...
describe("applyThreadFilter", () => {
  ...
  it("TRASH is always excluded regardless of settings", () => {
    const allOn: GmailSyncSettings = { includeSpam: true, includePromotions: true, sortingPaused: false, routeBulkToOther: true, labelWritebackEnabled: false, blacklistedSenderEmails: [] };
  ...
  it("SPAM is included when includeSpam is true", () => {
    const settings: GmailSyncSettings = { includeSpam: true, includePromotions: false, sortingPaused: false, routeBulkToOther: true, labelWritebackEnabled: false, blacklistedSenderEmails: [] };
    const snapshot = makeThread([makeMessage("msg-1", ["SPAM"])]);
  ...
  it("CATEGORY_PROMOTIONS is included when includePromotions is true", () => {
    const settings: GmailSyncSettings = { includeSpam: false, includePromotions: true, sortingPaused: false, routeBulkToOther: true, labelWritebackEnabled: false, blacklistedSenderEmails: [] };
    const snapshot = makeThread([makeMessage("msg-1", ["CATEGORY_PROMOTIONS", "INBOX"])]);
    expect(applyThreadFilter(snapshot, settings)).not.toBeNull();
  });
  ...
  it("TRASH is excluded even when message also has INBOX label", () => {
    ...
    expect(applyThreadFilter(snapshot, { includeSpam: true, includePromotions: true, sortingPaused: false, routeBulkToOther: true, labelWritebackEnabled: false, blacklistedSenderEmails: [] })).toBeNull();
  });
  ...
  it("SPAM and Promotions are treated independently", () => {
    ...
    const settings2: GmailSyncSettings = { includeSpam: false, includePromotions: true, sortingPaused: false, routeBulkToOther: true, labelWritebackEnabled: false, blacklistedSenderEmails: [] };
